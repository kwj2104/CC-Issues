...
1. `reports/composition.md` (+ a machine-readable `.csv` twin): pool size; top-1,000 vs full-pool distributions of age buckets (≤7d, 8–30d, 31–90d, >90d), bug/enhancement/other label mix, top-15 `area:*` labels, engagement deciles. **(rev 2)** an **exclusion waterfall** with counts at every step: open snapshot → in_pool → after each `exclude_label` (count per label) → after lock-reason filter → after junk filter → eligible (each step's remaining ≤ the previous; final remaining = the features-table eligible count); plus the count of `maintainer_authored` rows inside the top 1,000. **(rev 6)** the waterfall's `stale` step shows only non-rescued removals and a separate **"stale rescued: N"** line; plus **per-lane composition** (fills incl. spill, and age/rate_score/severity medians per lane), a **lane overlap matrix** (for each selected row, which other lanes' filters it also passes), and the **Jaccard of this lane head vs the old single-score head** (computed in memory). **(rev 4.1)** engagement-dominance is reported **per-lane** (the global top-100 is 100% big-bets, so a global flag is vacuous); a lane is flagged only if a rate/severity/cluster lane goes >90% engagement-dominated. Engagement deciles use **rank-based percentiles** (ties share a decile) with a note quantifying the zero-engagement collapse.
2. `reports/sensitivity.md` — **(rev 4.1) every variant re-runs the FULL lane portfolio** (lanes + spill + cluster dedup) and compares to the current run's **lane head** (not the retired single-score head); an unperturbed re-run must give Jaccard exactly 1.0 vs baseline (asserted). **(a) weight perturbation**: for each of the 6 weights, re-select with that weight ×0.5 and ×1.5 (12 variants; in memory, no DB writes) → Jaccard vs the baseline lane head, min/mean headline. **(b) creation-window variants (rev 3):** re-run the lane portfolio restricted to issues created within each `sensitivity.window_variants` value (90 and 180 days) of `snapshot_ts`; Jaccard vs the **no-window baseline** (on the 2026-07-22 snapshot: 90d ≈ 0.507, 180d ≈ 0.757).
